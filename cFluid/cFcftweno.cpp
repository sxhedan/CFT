...
void G_CARTESIAN::cft_WENO_flux(
        POINTER params,
        SWEEP *vst,
        FSWEEP *vflux,
	FSWEEP *cvflux,
        int n)
{
	...
	//For CFT.	Dan
	for (i = ghost_size; i < n+ghost_size+1; ++i)
	{
	    cvflux->dens_flux[i] = -lambda*flux[0][i];
	    cvflux->momn_flux[0][i] = -lambda*flux[1][i];
	    cvflux->momn_flux[1][i] = -lambda*flux[2][i];
	    cvflux->momn_flux[2][i] = -lambda*flux[3][i];
	    cvflux->engy_flux[i] = -lambda*flux[4][i];
	    cvflux->pdens_flux[0][i] = -lambda*flux[5][i];
	    cvflux->pdens_flux[1][i] = -lambda*flux[6][i];
	}
