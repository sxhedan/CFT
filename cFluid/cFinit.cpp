...
void G_CARTESIAN::initSodObliqProb(
        ...
        char *inname)
{
        ...
        CursorAfterString(infile,"Enter coefficients of interface polynomial:");
        for (i = 0; i<=level_func_params->max_degree; i++)
        {
            fscanf(infile, "%lf", &level_func_params->A[i]);
            (void) printf("%f", level_func_params->A[i]);
        }
        (void) printf("\n");

        eqn_params->level_func_params = (POINTER)level_func_params;
        level_func_pack->func_params = (POINTER)level_func_params;
        level_func_pack->func = level_linear2d_func;
...
void G_CARTESIAN::initVSTRMIntfc(
	LEVEL_FUNC_PACK *level_func_pack,
	char *inname)
{
	static FOURIER_POLY *level_func_params;
...
*/
	CursorAfterString(infile,"Enter number of modes in x direction:");
	fscanf(infile,"%d",&num_modes_x);
	(void) printf("%d\n",num_modes_x);
	CursorAfterString(infile,"Enter number of modes in y direction:");
	fscanf(infile,"%d",&num_modes_y);
	(void) printf("%d\n",num_modes_y);
	num_modes = num_modes_x + num_modes_y;
	level_func_params->num_modes = num_modes;
	FT_MatrixMemoryAlloc((POINTER*)&level_func_params->nu,num_modes,
				dim-1,sizeof(double));
	FT_VectorMemoryAlloc((POINTER*)&level_func_params->phase,num_modes,
	...
	if (dim == 2)
	{
	    printf("TWO_FLUID_VST_RM is for 3D only.\n");
	    clean_up(ERROR);
	}
	...
	FILE *ydata;
        ...
        sprintf(mesg,"Enter the name of the file containing the initial interface perturbation data in the x - direction:");
        CursorAfterString(infile,mesg);
        fscanf(infile,"%s",s);
        (void) printf("%s\n",s);
	...
	sprintf(mesg,"Enter the name of the file containing the initial interface perturbation data in the y - direction:");
	CursorAfterString(infile,mesg);
	fscanf(infile,"%s",s);
	(void) printf("%s\n",s);
	...
	double value;
        ...
        while(flag == 0)
        {
            ...
            {
                ungetc(c,xdata);
                fgets(s, 12, xdata);	//12	FIXME
                value = atof(s);
                if(count%3 == 0) //This is a wave number value
		{
		    value = 2*PI/value;	//wave length
		    value /= 10;	//from mm to cm
                    level_func_params->nu[count/3][0] = (U[0]-L[0])/value;	//frequency
                    level_func_params->nu[count/3][1] = 0.0;
		    printf("nu: %lf %lf. ", level_func_params->nu[count/3][0], level_func_params->nu[count/3][1]);
		}
                else if (count%3 == 1)	//This is an amplitude
                {
                    level_func_params->A[count/3] = -value/10;	//negative
		    printf("amplitude: %lf. ", level_func_params->A[count/3]);
/*
                    value = PI*drand48();
                    if(drand48() > 0.5)
                        value = -1*value;
...
*/
		...
		else if (count%3 == 2)	//This is a phase
		{
		    level_func_params->phase[count/3] = value*180/PI+270;
		    printf("phase: %lf.\n", level_func_params->phase[count/3]);
		}
                count++;
            }
        }
        ...
        while(flag == 1)
        {
            ...
            {
                ungetc(c,ydata);
                fgets(s, 12, ydata);	//12	FIXME
                value = atof(s);
                if(count%3 == 0) //This is a wave number value
		{
                    level_func_params->nu[count/3][0] = 0.0;
		    value = 2*PI/value;	//wave length
		    value /= 10;	//from mm to cm
                    level_func_params->nu[count/3][1] = (U[1]-L[1])/value;	//frequency
		    printf("nu: %lf %lf. ", level_func_params->nu[count/3][0], level_func_params->nu[count/3][1]);
		}
                else if (count%3 == 1)	//This is an amplitude
                {
...
void G_CARTESIAN::setRayleiTaylorParams(char *inname)
{
	...
	if (s[0] == 'y' || s[0] == 'Y')
        ...
        CursorAfterString(infile,"Type y to turn the parabolic step on:");
        fscanf(infile,"%s",str);
        (void) printf("%s\n",str);
        if (str[0] == 'y' || str[0] == 'Y')
        {
          eqn_params->parabolic_step = true;
          CursorAfterString(infile, "Enter NS viscosity coefficient (default 0):");
          fscanf(infile,"%lf",&eqn_params->mu);
          (void)printf("%f\n",eqn_params->mu);
          eqn_params->viscosity = true;
          if (eqn_params->mu == '\0')
          {
            eqn_params->viscosity = false;
          }
          //(void) printf("%f \n",&eqn_params->mu);


          CursorAfterString(infile, "Enter NS thermal conductivity coefficient (default 0):");
          fscanf(infile,"%lf",&eqn_params->kappa);
          (void)printf("%f\n",eqn_params->kappa);
          eqn_params->thermal_conduction = true;
          if (eqn_params->kappa == '\0')
          {
            eqn_params->thermal_conduction = false;
          }
          //(void) printf("%f \n",&eqn_params->kappa);

          CursorAfterString(infile, "Enter NS mass diffusivity coefficient (default 0):");
          ...
          if (str[0] == 'y' || str[0] == 'Y')
          {
            eqn_params->subgrid = true;
            CursorAfterString(infile,"Type y to have the subgrid viscosity (default n):");
            fscanf(infile,"%s",str);
            (void) printf("%s\n",str);
            if (str[0] == 'y' || str[0] == 'Y')
              eqn_params->subgrid_vis = true;
            else
              eqn_params->subgrid_vis = false;

            CursorAfterString(infile,"Type y to have the subgrid conductivity (default n):");
            fscanf(infile,"%s",str);
            (void) printf("%s\n",str);
            if (str[0] == 'y' || str[0] == 'Y')
              eqn_params->subgrid_con = true;
            else
              eqn_params->subgrid_con = false;

            CursorAfterString(infile,"Type y to have the subgrid mass diffusivity (default n):");
            fscanf(infile,"%s",str);
            (void) printf("%s\n",str);
            if (str[0] == 'y' || str[0] == 'Y')
              eqn_params->subgrid_md = true;
            else
              eqn_params->subgrid_md = false;

          }

          else
          {
            eqn_params->subgrid = false;
          }
...
void G_CARTESIAN::setVSTRMParams(char *inname)
{
	...
	CursorAfterString(infile,"Type y to turn the turbulent boundary layer on:");
	fscanf(infile,"%s",str);
	(void) printf("%s\n",str);
	if (str[0] == 'y' || str[0] == 'Y')
	{
	    ...
	    sprintf(str, "Enter viscosity of the fluid with comp %d:", GAS_COMP1);
	    CursorAfterString(infile,str);
	    fscanf(infile,"%lf",&((eqn_params->eos[GAS_COMP1]).tbl_visc));
	    (void) printf("%f\n",(eqn_params->eos[GAS_COMP1]).tbl_visc);
	    sprintf(str, "Enter thermal conductivity of the fluid with comp %d:", GAS_COMP1);
	    CursorAfterString(infile,str);
	    fscanf(infile,"%lf",&((eqn_params->eos[GAS_COMP1]).tbl_therm));
	    (void) printf("%f\n",(eqn_params->eos[GAS_COMP1]).tbl_therm);

	    sprintf(str, "Enter viscosity of the fluid with comp %d:", GAS_COMP2);
	    CursorAfterString(infile,str);
	    fscanf(infile,"%lf",&((eqn_params->eos[GAS_COMP2]).tbl_visc));
	    (void) printf("%f\n",(eqn_params->eos[GAS_COMP2]).tbl_visc);
	    sprintf(str, "Enter thermal conductivity of the fluid with comp %d:", GAS_COMP2);
	    CursorAfterString(infile,str);
	    fscanf(infile,"%lf",&((eqn_params->eos[GAS_COMP2]).tbl_therm));
	    (void) printf("%f\n",(eqn_params->eos[GAS_COMP2]).tbl_therm);
	}
	else
	{
	    eqn_params->TBL = false;
	}
