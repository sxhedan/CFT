...
case "$HOST" in   
	...
	w*) host_name=seawulf ;;
	a*) host_name=ncsa ;;
	*) echo "ERROR: Must run run_autotests.sh in interactive mode." 
	exit 0 ;;
esac 
...
then
	echo "Started run_autotests.sh on $host_name:"
	date
	echo ""
else
	echo "ERROR: The hostname provided is not recognized."
   	exit 0
fi
...
do
	cd $i
	./$i.sh $host_name > ../$i.out 2>&1
	cd ..
	echo "Completed $i:"
	date
	echo ""
	echo "Note: Please refer to $i.out for test results."
	echo "Please refer to ./$i/out/test_name/test_name.out"
	echo "for individual test outputs."
	echo ""
done
echo "Finished run_autotests.sh on $host_name:"
date
echo ""