...
echo "========================================================================="
echo "autotest3d - performs tests provided in the \"in\" directory, using the "
...
do
    ...
    then
	mpirun -np 4 $PATH_/../../iFluid -d 3 -p 2 2 1 -i $PATH_/in/$i \
	    > $i.out 2>/dev/null
    elif [ "$HOST" == "ncsa" ]
    then
	mpirun $PATH_/../../iFluid -d 3 -i $PATH_/in/$i -np 4 \
	    > $i.out 2>/dev/null 
    else
...
do
	...
	if [ ! -d "archive/${i}" ]
		...
		compare_cgns('$PATH_/out/${i}/intfc/cgns.ts0000${tsmx}-nd000${j}/3d-intfc.cgns',\
		'$PATH_/archive/${i}/intfc/cgns.ts0000${tsmx}-nd000${j}/3d-intfc.cgns');"`
